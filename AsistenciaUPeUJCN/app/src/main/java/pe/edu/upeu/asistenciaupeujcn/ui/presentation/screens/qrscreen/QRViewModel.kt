...
class QRViewModel @Inject constructor(
    ...
    fun updateBarcodes(barcodes: List<Barcode>, width: Int, height: Int) {
        if (isProcessing) return  // No permitir procesamiento si ya está en curso
            ...
            try {
                if (barcodes.size == 1) {
                    ...
                    if (dato != null && dato != lastProcessedBarcode) {
                        isProcessing = true // Marcar como procesamiento en curso
                        ...
                        if (success) {
                            ...
                            _insertStatus.postValue(true) // Notificar éxito
                            resetLastProcessedBarcode()
                        }else {
                            _insertStatus.postValue(false) // Notificar error
                        }
                        }else{
                            _insertStatus.postValue(false) // Notificar error
                        }
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(p0: LocationResult) {
                if (p0.locations.isNotEmpty()) {
                    ...
                    // Remover actualizaciones de ubicación después de obtener la primera ubicación
